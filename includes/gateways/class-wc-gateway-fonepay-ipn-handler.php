...
class WC_Gateway_Fonepay_IPN_Handler extends WC_Gateway_Fonepay_Response
{
    ...
    {
        add_action(
            'woocommerce_api_wc_gateway_fonepay',
            array($this, 'check_response')
        );

        add_action(
            'valid-fonepay-standard-ipn-request',
    ...
    public function valid_response($response)
    {
        ...
        if ($order) {
            ...
            if (method_exists($this, "payment_status_$payment_status")) {
                call_user_func(
                    array(
                        $this, "payment_status_$payment_status"
                    ),
                    $order,
                    $response
                );
    ...
    protected function payment_status_completed($order, $response)
    {
        ...
        $this->payment_complete(
            $order,
            wc_clean($response['UID']),
            __('IPN payment completed', 'woocommerce-fonepay')
        );
    ...
    protected function payment_status_failed($order, $response)
    {
        $order->update_status(
            ...
            sprintf(__('Payment %s via IPN.', 'woocommerce-fonepay'), 'failed')
        );
    }
            ...
            $this->send_ipn_email_notification(
                ...
                sprintf(
                    __(
                        ...
                        'Order #%s has been marked paid by Fonepay IPN, but was previously cancelled. Admin handling required.',
                        'woocommerce-fonepay'
                    ),
